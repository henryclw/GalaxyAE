...
def select_by_param(data, param, internal_min=None, internal_max=None):
    ...
    if internal_min != None:
        min_mask = (data[param] >= internal_min)
    else:
        min_mask = np.ones(len(data), dtype=bool)

    if internal_max != None:
        max_mask = (data[param] <= internal_max)
    else:
        max_mask = np.ones(len(data), dtype=bool)
